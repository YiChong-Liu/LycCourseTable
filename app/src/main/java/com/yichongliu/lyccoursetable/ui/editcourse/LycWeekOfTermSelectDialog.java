...
public class LycWeekOfTermSelectDialog extends Dialog {
    private List<Boolean> list = new ArrayList<>(LycConfig.getMaxWeekNum());
    ...
    public LycWeekOfTermSelectDialog(@NonNull Context context, int weekOfTerm) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.dialog_lyc_select_week_of_term);
        init();
    ...
    private void init() {
        if (mWeekOfTerm == -1) {
            for (int i = 0, len = LycConfig.getMaxWeekNum(); i < len; i++) {
                list.add(false);
            }
        } else {
