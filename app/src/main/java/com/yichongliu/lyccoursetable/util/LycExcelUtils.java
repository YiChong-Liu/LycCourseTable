package com.yichongliu.lyccoursetable.util;
...
public class LycExcelUtils {
    public static interface HandleResult {
        LycCourse handle(String courseStr, int row, int col);
    }
     ...
     * 只读取6行7列
     */
    public static List<LycCourse> handleExcel(String path, int startRow, int startCol, HandleResult handleResult) {
        InputStream inputStream = null;
        List<LycCourse> courseList = new ArrayList<>();
        //Log.d("filePath",path);
        Workbook excel = null;
        try {
                ...
                for (int j = 1; j <= rowCount; j++) {
                    ...
                    if (!str.isEmpty()) {
                        ...
                        for (String s : strings) {
                            LycCourse course = handleResult.handle(s, j, i);
                            if (course != null) {
    ...
    public static List<LycCourse> handleExcel(String path, int startRow, int startCol) {
        return handleExcel(path, startRow, startCol, new HandleResult() {
            @Override
            public LycCourse handle(String courseStr, int row, int col) {
                LycCourse course = getCourseFromString(courseStr);
                if (course == null)
                    return null;
    ...
    public static List<LycCourse> handleExcel(String path) {
     ...
     * @return Course
     */
    public static LycCourse getCourseFromString(String str) {

        String[] contents = str.split("\n");
        if (contents.length < 4)
            return null;
        LycCourse course = new LycCourse();
        ...
        for (String s : s11) {
            ...
            if (s.contains("-")) {
                ...
                for (int n = p; n <= q; n += space) {
                    weekOfTerm += 1 << (LycConfig.getMaxWeekNum() - n);
                }
            } else {
                weekOfTerm += 1 << (LycConfig.getMaxWeekNum() - Integer.parseInt(s));
            }
        }
        return weekOfTerm;
    }
